...
class ExposurePSF:
    ...
    def neEval(self, s, **kwargs):
        ...
        #return numexpr.evaluate(s, local_dict=kwargs)
    ...
    def comp(self, param, normed=False):
        ...
        r2 = self.neEval("dx**2 + ell*dy**2 + 2*PA*dx*dy",
                         dx=dx, dy=dy, ell=ell, PA=PA)
        gaussian = self.neEval("exp(-r2/2/sigma**2)",
                               r2=r2, sigma=sigma)
        moffat = self.neEval("(1 + r2/alpha**2)**(-beta)",
                             r2=r2, alpha=alpha, beta=beta)

        # Function
        val = self.neEval("moffat + eta*gaussian",
                          moffat=moffat, eta=eta, gaussian=gaussian)
        val *= self.param[self.npar_cor:,N.newaxis] 
    ...
    def deriv(self, param):
        ...
        dx = self.x - x0
        dy = self.y - y0
        dy2 = self.neEval("dy**2", dy=dy)
        r2 = self.neEval("dx**2 + ell*dy**2 + 2*PA*dx*dy",
                         dx=dx, dy=dy, ell=ell, PA=PA)
        gaussian = self.neEval("exp(-r2/2/sigma**2)", r2=r2, sigma=sigma)
        ea = self.neEval("1 + r2/alpha**2", r2=r2, alpha=alpha)
        moffat = self.neEval("ea**(-beta)", ea=ea, beta=beta)
        j1 = self.neEval("eta/sigma**2", eta=eta, sigma=sigma)
        j2 = self.neEval("2*beta/ea/alpha**2", beta=beta, ea=ea, alpha=alpha)
        da0 = self.neEval("gaussian * ( e1 + s1*r2*j1/sigma ) + " \
                          "moffat * ( -b1*log(ea) + r2*j2/alpha )",
                          gaussian=gaussian, moffat=moffat,
                          e1=e1, s1=s1, r2=r2, b1=b1, j1=j1, j2=j2,
                          sigma=sigma, ea=ea, alpha=alpha)

        # Derivatives
        grad = N.zeros((self.npar_cor+self.npar_ind,)+self.x.shape, 'd')
        tmp = self.neEval("gaussian*j1 + moffat*j2",
                          gaussian=gaussian, moffat=moffat, j1=j1, j2=j2)
        grad[2] = self.neEval("tmp*(dx + PA*dy)",
                              tmp=tmp, dx=dx, dy=dy, PA=PA)  # dPSF/dx0
        grad[3] = self.neEval("tmp*(ell*dy + PA*dx)",
                              tmp=tmp, dx=dx, dy=dy, PA=PA, ell=ell)  # dPSF/dy0
        grad[0] = self.neEval("c*(sintheta*g2 - costheta*g3)",
                              c=self.ADR_coeff,
                              g2=grad[2], g3=grad[3],
                              sintheta=sintheta, costheta=costheta)
        grad[1] = self.neEval("delta*c*(sintheta*g3 + costheta*g2)",
                              c=self.ADR_coeff,
                              g2=grad[2], g3=grad[3],
                              sintheta=sintheta, costheta=costheta, delta=delta)
        grad[4] = self.neEval("-tmp * dx*dy",
                              tmp=tmp, dx=dx, dy=dy)        # dPSF/dPA
        for i in xrange(self.ellDeg + 1):
            grad[5+i] = self.neEval("-tmp/2 * dy2 * lrel**i",
                                    tmp=tmp, dy2=dy2, i=i, lrel=self.lrel)
        for i in xrange(self.alphaDeg + 1):
            grad[6+self.ellDeg+i] = self.neEval("da0 * lrel**i",
                                                da0=da0, i=i, lrel=self.lrel)
        grad[:self.npar_cor] *= self.param[N.newaxis,self.npar_cor:,N.newaxis]
        grad[self.npar_cor] = self.neEval("moffat + eta*gaussian",
                                          moffat=moffat, gaussian=gaussian,
                                          eta=eta) # dPSF/dI
