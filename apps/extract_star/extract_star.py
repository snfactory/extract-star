...
def get_start(cube, psf_fn, skyDeg=0):
    ...
    for i in xrange(nslice):
        ...
        model_star = pySNIFS_fit.model(data=cube_star,
                                       func=['%s;%f,%f,%f,%f' % \
                                             (psf_fn.__name__,
                                              SpaxelSize,lbda_ref,0,0), # a0=cte,ell=cte
                                             'poly2D;%d' % skyDeg],
                                       param=[p1,p2],
                                       bounds=[b1,b2],
